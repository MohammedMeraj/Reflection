...
export default function ProfilePage() {
  ...
  // Show skeleton loading until department head data is loaded
  if (!departmentHead) {
    return (
      ...
      <div className="pt-20 pb-20 p-4 max-w-md mx-auto space-y-6">
        ...
        <div className="bg-white rounded-lg p-6 shadow-sm">
          <div className="flex flex-col items-center">
            ...
            <h2 className="mt-4 text-xl font-bold text-gray-800 break-words text-center">{departmentHead.name}</h2>
            <p className="text-sm text-gray-600 break-words text-center">{departmentHead.department?.name}</p>
          </div>
        </div>
          ...
          <div className="space-y-4">
            <div>
              ...
              {isEditing ? (
                ...
                <p className="text-gray-800 break-words overflow-wrap-anywhere">{departmentHead.name}</p>
              )}
            </div>

            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Email (Read-only)</label>
              <p className="text-gray-500 bg-gray-50 p-2 rounded-md break-all text-sm">{departmentHead.email}</p>
            </div>

            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Employee ID (Read-only)</label>
              <p className="text-gray-500 bg-gray-50 p-2 rounded-md break-words text-sm">{departmentHead.employeeId}</p>
            </div>
            ...
            <div>
              ...
              {isEditing ? (
                ...
                <p className="text-gray-800 break-words">{departmentHead.phone || 'Not provided'}</p>
              )}
            </div>
            ...
            <div>
              ...
              {isEditing ? (
                ...
                <p className="text-gray-800 break-words whitespace-pre-wrap">{departmentHead.qualification || 'Not provided'}</p>
              )}
            </div>
        ...
        <div className="bg-white rounded-lg p-6 shadow-sm">
          ...
          <div className="space-y-3">
            <div className="flex flex-col sm:flex-row sm:justify-between gap-1">
              ...
              <span className="text-sm text-gray-800 break-words text-right">{departmentHead.department?.name}</span>
            </div>
            <div className="flex flex-col sm:flex-row sm:justify-between gap-1">
              <span className="text-sm font-medium text-gray-700">Department Code:</span>
              <span className="text-sm text-gray-800 break-words text-right">{departmentHead.department?.code}</span>
            </div>
            <div className="flex flex-col sm:flex-row sm:justify-between gap-1">
              <span className="text-sm font-medium text-gray-700">Status:</span>
              <span className={`text-sm px-2 py-1 rounded-full w-fit ml-auto ${departmentHead.isActive ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
      ...
      {showConfirmation && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg p-6 max-w-sm w-full mx-4">
            <h3 className="text-lg font-semibold text-gray-800 mb-4 break-words">Confirm Changes</h3>
